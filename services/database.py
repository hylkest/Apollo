...
class Database:
    ...
    def add_link_with_category(self, url, category):
        ...
        cursor = conn.cursor()
        sql = f"INSERT IGNORE INTO links (url, category) VALUES (%s, %s)"
        cursor.execute(sql, url, category)
        print(cursor.execute(sql, url, category))
    ...
    def get_all_links_by_category(self, category):
        conn = mysql.connector.connect(**self.config)
        cursor = conn.cursor(dictionary=True)
        sql = f"SELECT * FROM links WHERE category = '%s'"
    ...
    def add_category(self, category):
        conn = mysql.connector.connect(**self.config)
        cursor = conn.cursor()
        sql = f"INSERT INTO categories (category_name) VALUES (%s)"
        cursor.execute(sql, category)
        conn.commit()
        cursor.close()
        conn.close()
    ...
    def save_url_title(self, url_id, title):
        conn = mysql.connector.connect(**self.config)
        cursor = conn.cursor()
        sql = f"INSERT INTO url_title (url_id, url_title) VALUES (%s, %s)"
